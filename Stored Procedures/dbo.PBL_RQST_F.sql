...
*/
      ...
      BEGIN
       SELECT @DiseCode = r.query('Fighter_Public/Dise_Code').value('.', 'BIGINT')
             ...
             ,@BrthPlac = @x.query('Fighter_Public/Brth_Plac').value('.', 'NVARCHAR(100)')
             ,@IssuPlac = @x.query('Fighter_Public/Issu_Plac').value('.', 'NVARCHAR(100)')
             ,@FathWork = @x.query('Fighter_Public/Fath_Work').value('.', 'NVARCHAR(150)')
             ,@HistDesc = @x.query('Fighter_Public/Hist_Desc').value('.', 'NVARCHAR(500)')
             ,@IntrFileNo = @x.query('Fighter_Public/Intr_File_No').value('.', 'BIGINT')             
             ,@CntrCode = @x.query('Fighter_Public/Cntr_Code').value('.', 'BIGINT')             
             ,@DpstAcntSlryBank = @x.query('Fighter_Public/Dpst_Acnt_Slry_Bank').value('.', 'NVARCHAR(50)')             
             ,@DpstAcntSlry = @x.query('Fighter_Public/Dpst_Acnt_Slry').value('.', 'VARCHAR(50)')             
         FROM @X.nodes('//Request_Row')Rr(r)
