...
-- =============================================
CREATE PROCEDURE [dbo].[OIC_ICRT_P]
	@X XML
AS
BEGIN
   DECLARE @AP BIT
   ...
   BEGIN TRAN T1
      ...
      BEGIN
         ...
         -- 1396/2/10 * برای مشخص شدن درخواست تمدید جلسات ترکیبی 
         IF @SendExpn = '001'
            SET @RqttCode = '004'
         ELSE
            SET @RqttCode = '008';
         ...
         BEGIN
            --IF NOT EXISTS(
            --   SELECT S.MBSP_FIGH_FILE_NO, '001', S.EXPN_CODE, dbo.GNRT_NVID_U(), S.SESN_TYPE, s.TOTL_SESN - S.SUM_MEET_HELD_DNRM, s.CARD_NUMB, s.CBMT_CODE
            ...
            SELECT S.MBSP_FIGH_FILE_NO, '001', @MbspRwno, S.EXPN_CODE, dbo.GNRT_NVID_U(), S.SESN_TYPE, /*s.TOTL_SESN - ISNULL(S.SUM_MEET_HELD_DNRM, 0)*/ 0, s.CARD_NUMB, s.CBMT_CODE, SNID
              FROM dbo.Session S, Member_Ship M, Fighter F
         ...
         BEGIN
            ...
            BEGIN
               UPDATE [Session]
                  SET EXPN_CODE = @ExpnCode
                     ,CARD_NUMB = @FngrPrnt
                     ,TOTL_SESN = @TotlSesn/*CASE @SendExpn WHEN '001' THEN @TotlSesn ELSE 0 END*/
                     ,CBMT_CODE = @CbmtCode
                WHERE MBSP_FIGH_FILE_NO = @FileNo
                  AND MBSP_RECT_CODE = '001'
                  AND MBSP_RWNO = @MbspRwno
            ...
            -- 1396/05/09 * برای بروز کردن تعداد کل جلسات
         ...
         -- 1396/05/09 * بروز رسانی جدول برای تعداد کل جلسات
         UPDATE Member_Ship
