...
BEGIN
	...
	SET @AccessString = N'<AP><UserName>' + SUSER_NAME() + '</UserName><Privilege>8</Privilege><Sub_Sys>5</Sub_Sys></AP>';	
   EXEC iProject.dbo.SP_EXECUTESQL N'SELECT @ap = DataGuard.AccessPrivilege(@P1)',N'@P1 ntext, @ap BIT OUTPUT',@AccessString , @ap = @ap output
   IF @AP = 0 
   BEGIN
      RAISERROR ( N'خطا - عدم دسترسی به ردیف 8 سطوح امینتی : شما مجوز ایجاد ترکیب تقاضا و متقاضی برای آیین نامه مدارک و مجوزات را ندارید', -- Message text.
               16, -- Severity.
               1 -- State.
               );
      RETURN;
