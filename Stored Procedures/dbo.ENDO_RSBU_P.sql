...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         IF @CellPhon IS NOT NULL AND (SELECT COUNT(*) from dbo.Fighter where CELL_PHON_DNRM = @CellPhon) = 1
         BEGIN
         ...
         BEGIN
               ...
               SELECT '025' AS '@rqtpcode'
                     ...
                     ,(
                        SELECT N'آقای' AS 'Last_Name'
                              ,ISNULL(CUST_NAME, '') as 'Frst_Name'
                              ,ISNULL(CELL_PHON, '') AS 'Cell_Phon'
                          FROM dbo.Aggregation_Operation_Detail
                         WHERE AGOP_CODE = @AgopCode
                           AND RWNO = @Rwno
            ...
            SELECT TOP 1
                   @FileNo = F.FILE_NO
                  ,@Rqid = R.RQID
              FROM Request r, dbo.Request_Row rr, dbo.Fighter f
   ...
   INSERT INTO dbo.Payment_Detail 
   ( PYMT_CASH_CODE , PYMT_RQST_RQID , RQRO_RWNO , EXPN_CODE , CODE , PAY_STAT , EXPN_PRIC , EXPN_EXTR_PRCT , QNTY)
   SELECT @CashCode, @Rqid, 1, B.EXPN_CODE, dbo.GNRT_NVID_U(), '001', E.PRIC, E.EXTR_PRCT, B.QNTY
     FROM dbo.Buffet B, dbo.Expense E
   ...
   IF (ISNULL(@Paym, 0) + ISNULL(@Pyds, 0)) = ISNULL(@TotlAmntDnrm, 0)
      SELECT @X = (
         SELECT @Rqid AS '@rqid'
               ,(
                  SELECT @FileNo AS '@fileno'
  ...
  END TRY
  BEGIN CATCH
      IF (SELECT CURSOR_STATUS('local','C$Pyds')) >= -1
      BEGIN
        IF (SELECT CURSOR_STATUS('local','C$Pyds')) > -1
         BEGIN
          CLOSE C$Pyds
       ...
       DEALLOCATE C$Pyds
      END
      IF (SELECT CURSOR_STATUS('local','C$Paym')) >= -1
      BEGIN
        IF (SELECT CURSOR_STATUS('local','C$Paym')) > -1
         BEGIN
          CLOSE C$Paym
