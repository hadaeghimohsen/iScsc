...
-- =============================================
CREATE PROCEDURE [dbo].[PSF_RQST_P]
	@X XML
AS
BEGIN
   DECLARE @AP BIT
          ,@AccessString VARCHAR(250);
   SET @AccessString = N'<AP><UserName>' + SUSER_NAME() + '</UserName><Privilege>82</Privilege><Sub_Sys>5</Sub_Sys></AP>';	
   EXEC iProject.dbo.SP_EXECUTESQL N'SELECT @ap = DataGuard.AccessPrivilege(@P1)',N'@P1 ntext, @ap BIT OUTPUT',@AccessString , @ap = @ap output
   IF @AP = 0 
   BEGIN
      RAISERROR ( N'خطا - عدم دسترسی به ردیف 82 سطوح امینتی', -- Message text.
               16, -- Severity.
               1 -- State.
               );
      RETURN;
   ...
   SET @AccessString = N'<AP><UserName>' + SUSER_NAME() + '</UserName><Privilege>83</Privilege><Sub_Sys>5</Sub_Sys></AP>';	
   EXEC iProject.dbo.SP_EXECUTESQL N'SELECT @ap = DataGuard.AccessPrivilege(@P1)',N'@P1 ntext, @ap BIT OUTPUT',@AccessString , @ap = @ap output
   IF @AP = 0 
   BEGIN
      RAISERROR ( N'خطا - عدم دسترسی به ردیف 83 سطوح امینتی', -- Message text.
               16, -- Severity.
               1 -- State.
               );
      RETURN;
   ...
   SET @AccessString = N'<AP><UserName>' + SUSER_NAME() + '</UserName><Privilege>84</Privilege><Sub_Sys>5</Sub_Sys></AP>';	
   EXEC iProject.dbo.SP_EXECUTESQL N'SELECT @ap = DataGuard.AccessPrivilege(@P1)',N'@P1 ntext, @ap BIT OUTPUT',@AccessString , @ap = @ap output
   IF @AP = 0 
   BEGIN
      RAISERROR ( N'خطا - عدم دسترسی به ردیف 84 سطوح امینتی', -- Message text.
               16, -- Severity.
               1 -- State.
               );
      RETURN;
	...
	BEGIN TRY
      ...
      DECLARE @FileNo   BIGINT
	   ...
	   SELECT @Rqid     = @X.query('//Request').value('(Request/@rqid)[1]'    , 'BIGINT')
	         ...
	         ,@PullUp   = @X.query('//Physical_Fitness').value('(Physical_Fitness/@pullup)[1]', 'REAL')
	         ,@PushUp   = @X.query('//Physical_Fitness').value('(Physical_Fitness/@pushup)[1]', 'REAL')
	         ,@SqutTrst = @X.query('//Physical_Fitness').value('(Physical_Fitness/@squttrst)[1]', 'REAL')
	         ,@SqutJump = @X.query('//Physical_Fitness').value('(Physical_Fitness/@squtjump)[1]', 'REAL')
	         ,@SitUp    = @X.query('//Physical_Fitness').value('(Physical_Fitness/@situp)[1]', 'REAL');
