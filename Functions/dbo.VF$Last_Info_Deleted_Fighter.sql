...
SET ANSI_NULLS ON
GO
CREATE FUNCTION [dbo].[VF$Last_Info_Deleted_Fighter] (
   @FileNo BIGINT,
...
WHERE (f.CONF_STAT = '002')
  ...
  AND (f.ACTV_TAG_DNRM <= '100')
  AND (@FileNo IS NULL OR f.FILE_NO = @FileNo)
  AND (@FrstName IS NULL OR @FrstName = '' OR f.NAME_DNRM LIKE N'%' + @FrstName + N'%')
  AND (@LastName IS NULL OR @LastName = '' OR f.NAME_DNRM LIKE N'%' + @LastName + N'%')
  AND (@NatlCode IS NULL OR @NatlCode = '' OR f.NATL_CODE_DNRM LIKE @NatlCode)
  AND (@FngrPrnt IS NULL OR @FngrPrnt = '' OR f.FNGR_PRNT_DNRM LIKE @FngrPrnt)
  AND (@CellPhon IS NULL OR @CellPhon = '' OR f.CELL_PHON_DNRM LIKE @CellPhon)
  AND (@TellPhon IS NULL OR @TellPhon = '' OR f.TELL_PHON_DNRM LIKE @TellPhon)
