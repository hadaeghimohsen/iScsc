...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[CG$AINS_ATTN]
   ON  [dbo].[Attendance]
   AFTER INSERT
AS 
BEGIN
   ...
   USING (SELECT i.CLUB_CODE, i.FIGH_FILE_NO, i.ATTN_DATE, i.CODE, f.MBCO_RWNO_DNRM, m.NUMB_OF_ATTN_MONT, m.SUM_ATTN_MONT_DNRM, f.BRTH_DATE_DNRM, f.MBSP_RWNO_DNRM, f.NAME_DNRM, f.CELL_PHON_DNRM, f.FGPB_TYPE_DNRM, ISNULL(f.MTOD_CODE_DNRM, i.MTOD_CODE_DNRM) AS MTOD_CODE_DNRM, ISNULL(f.CTGY_CODE_DNRM, i.CTGY_CODE_DNRM) AS CTGY_CODE_DNRM, f.IMAG_RCDC_RCID_DNRM, f.IMAG_RWNO_DNRM, f.FNGR_PRNT_DNRM, f.DEBT_DNRM, f.BUFE_DEBT_DNTM, f.MBSP_STRT_DATE, f.MBSP_END_DATE 
            FROM INSERTED i, dbo.Fighter f LEFT OUTER join dbo.Member_Ship m ON m.FIGH_FILE_NO = f.FILE_NO AND m.RWNO = f.MBSP_RWNO_DNRM AND m.RECT_CODE = '004'
           WHERE i.FIGH_FILE_NO = f.FILE_NO ) S
   ON (T.CLUB_CODE                  = S.CLUB_CODE AND
       T.FIGH_FILE_NO               = S.FIGH_FILE_NO AND
       CAST(T.Attn_Date AS DATE)    = CAST(S.Attn_Date AS DATE) AND
       T.CODE = S.Code)
   WHEN MATCHED THEN
      UPDATE
         SET T.CRET_BY   = UPPER(SUSER_NAME())
            ...
            ,T.MBSP_RECT_CODE_DNRM = CASE WHEN /*EXISTS (SELECT * FROM dbo.Fighter WHERE FILE_NO = S.Figh_File_No AND*/ S.FGPB_TYPE_DNRM NOT IN ('002', '003') THEN '004' ELSE NULL END
            ...
            ,T.SUM_ATTN_MONT_DNRM = CASE WHEN s.FGPB_TYPE_DNRM NOT IN ('002', '003') THEN ISNULL(S.SUM_ATTN_MONT_DNRM, 0) + 1 ELSE NULL END
            ,T.MBCO_RWNO_DNRM = S.MBCO_RWNO_DNRM;
   
   -- اختصاص کلید کمد به کاربر اگر سامانه کمد فعال باشد و به صورت اتوماتیک انجام شود
   IF EXISTS(SELECT * FROM Attendance A, Inserted I WHERE A.CODE = I.CODE AND A.EXIT_TIME IS NULL) AND
      ...
      IF EXISTS(
       SELECT *
          ...
          WHERE f.FILE_NO = m.FIGH_FILE_NO
            ...
            AND 
            (
               ( ISNULL(m.NUMB_OF_ATTN_MONT, 0) >= 1 AND ISNULL(m.NUMB_OF_ATTN_MONT, 0) > ISNULL(m.SUM_ATTN_MONT_DNRM, 0) ) OR 
               ( ISNULL(m.NUMB_OF_ATTN_MONT, 0) = 0 )
            )
      )
      BEGIN         
         UPDATE dbo.Member_Ship
          ...
          WHERE EXISTS(
            SELECT * FROM INSERTED I, Attendance A
            WHERE dbo.Member_Ship.FIGH_FILE_NO = A.Figh_File_No
              ...
              AND ((A.ATTN_TYPE <> '002' AND A.EXIT_Time IS NULL)
                OR (A.ATTN_TYPE = '002' AND A.EXIT_Time IS NOT NULL))
          );
      END
      ELSE
      BEGIN
 ...
 IF EXISTS(
   ...
   BEGIN
      ...
      IF EXISTS(
         SELECT * 
           FROM Fighter F, Member_Ship M, inserted I
          WHERE F.FILE_NO = M.FIGH_FILE_NO
         ...
         IF EXISTS(
            SELECT * 
             ...
             WHERE F.FILE_NO = M.FIGH_FILE_NO
               ...
               AND M.RECT_CODE = '004'
               AND DATEDIFF(DAY, M.STRT_DATE, M.END_DATE) = 0
               AND S.TOTL_SESN > ISNULL(S.SUM_MEET_HELD_DNRM, 0)
         )
         BEGIN
            MERGE Session_Meeting T
            USING ( SELECT S.MBSP_FIGH_FILE_NO, S.MBSP_RECT_CODE, S.MBSP_RWNO , S.EXPN_CODE, S.SNID, 0 AS RWNO, '002' AS VALD_TYPE, GETDATE() AS STRT_TIME, DATEADD(MINUTE, 90, GETDATE()) AS END_TIME, 1 AS NUMB_OF_GAYS
                      FROM Fighter F, Member_Ship M, [Session] S, Inserted I
                     WHERE F.FILE_NO = I.FIGH_FILE_NO
            ...
            WHEN NOT MATCHED THEN
               INSERT (MBSP_FIGH_FILE_NO, MBSP_RECT_CODE, MBSP_RWNO, EXPN_CODE, SESN_SNID, RWNO, VALD_TYPE, STRT_TIME, END_TIME, NUMB_OF_GAYS)
               VALUES (S.MBSP_FIGH_FILE_NO, S.MBSP_RECT_CODE, S.MBSP_RWNO , S.EXPN_CODE, S.SNID, 0, '002', GETDATE(), DATEADD(MINUTE, 90, GETDATE()), 1)
            WHEN MATCHED THEN
               UPDATE 
                  SET NUMB_OF_GAYS = S.NUMB_OF_GAYS + 1;
            ...
            -- اختصاص کلید کمد به کاربر اگر سامانه کمد فعال باشد و به صورت اتوماتیک انجام شود
            IF EXISTS(SELECT * FROM Settings WHERE DRES_STAT = '002' AND DRES_AUTO = '002')
            BEGIN      
         ...
         IF EXISTS(
            SELECT * 
             ...
             WHERE F.FILE_NO = M.FIGH_FILE_NO
               ...
               AND M.RECT_CODE = '004'
               AND DATEDIFF(DAY, M.STRT_DATE, M.END_DATE) >= 1
               AND (S.TOTL_SESN + 2)> ISNULL(S.SUM_MEET_HELD_DNRM, 0)
         )
         BEGIN
            IF (
               EXISTS(
                  SELECT *
                    FROM Settings S, Fighter F, inserted I
                   WHERE S.CLUB_CODE = F.CLUB_CODE_DNRM
                     AND F.FILE_NO = I.FIGH_FILE_NO
                     AND S.MORE_ATTN_SESN = '001' -- تک جلسه در روز
               ) AND NOT EXISTS(
                  SELECT *
                    FROM Fighter F, Member_Ship M, [Session] S, Session_Meeting Sm, Inserted I
                   WHERE F.FILE_NO = I.FIGH_FILE_NO
                     AND F.FILE_NO = M.FIGH_FILE_NO
                     AND F.MBSP_RWNO_DNRM = M.RWNO
                     AND M.RECT_CODE = '004'
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[CG$AUPD_ATTN]
   ON  [dbo].[Attendance]
   AFTER UPDATE
AS 
BEGIN
...
/*   IF EXISTS(SELECT * FROM Attendance A, Inserted I WHERE A.CODE = I.CODE AND A.EXIT_TIME IS NOT NULL) AND
   ...
   IF EXISTS(
      SELECT * 
        FROM Fighter F, Inserted I, DELETED D
       WHERE F.FILE_NO = I.FIGH_FILE_NO
   ...
   BEGIN
      ...
      IF EXISTS(
         SELECT *
           FROM dbo.Fighter f, dbo.Member_Ship m, Attendance A, INSERTED i, DELETED D
          WHERE f.FILE_NO = m.FIGH_FILE_NO
            AND f.MBSP_RWNO_DNRM = m.RWNO
            AND m.RECT_CODE = '004'
            AND A.Figh_File_No = m.FIGH_FILE_NO
      ...
      BEGIN         
         UPDATE dbo.Member_Ship
            SET SUM_ATTN_MONT_DNRM = ISNULL(SUM_ATTN_MONT_DNRM, 0) - 1
               --,SUM_ATTN_WEEK_DNRM = dbo.GET_SATN_F('<Fighter fileno="' + CAST(Figh_File_No AS VARCHAR(14)) + '"/>').query('//Attendance').value('(Attendance/@d)[1]', 'BIGINT')
          WHERE EXISTS(
            SELECT * FROM INSERTED I, Attendance A, DELETED D
            WHERE dbo.Member_Ship.FIGH_FILE_NO = A.Figh_File_No
              AND dbo.Member_Ship.RWNO = A.Mbsp_Rwno_Dnrm
              AND dbo.Member_Ship.RECT_CODE = A.Mbsp_Rect_Code_Dnrm
              AND A.FIGH_FILE_NO = I.FIGH_FILE_NO
...
AND I.Code = D.Code
   ...
   IF EXISTS(
	  SELECT *
	    FROM dbo.Fighter f, Inserted I, dbo.Settings S
	   WHERE F.FILE_NO = I.FIGH_FILE_NO
   ...
   BEGIN
	  ...
	  SELECT @ErrMsg = 
			 N' تداخل در ثبت شماره کمد ' + CAST(@DersNumb AS NVARCHAR(10)) + N' ' + CHAR(10) +
			 N' شماره کمد مورد نظر آخرین بار توسط ' + @NameDnrm + N' در تاریخ ' + @AttnDate + N' گرفته شده و پس داده نشده ' + CHAR(10) +
			 CASE WHEN COALESCE(@CellPhonDnrm,'')= ''  THEN N' متاسفانه برای پیگیری شماره ای در سیستم ثبت نشده '
			      ELSE N' برای پیگیری می توانید با این شماره ' + @CellPhonDnrm + N' تماس حاصل کنید '
			 END
