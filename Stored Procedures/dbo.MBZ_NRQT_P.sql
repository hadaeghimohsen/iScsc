...
-- =============================================
CREATE PROCEDURE [dbo].[MBZ_NRQT_P]
...
BEGIN
	...
	*/
	...
	BEGIN TRY
      ...
      BEGIN
         RAISERROR(N'مدت زمان بلوکه کردن کارت داوطلب به پایان رسیده، نیازی به آزاد کردن کارت داوطلب نیست', 16, 1);
         RETURN 0;
      END
      ...
      SET @StrtDate = DATEADD(DAY, -1, GETDATE());
      ...
      -- 1396/05/09 * برای بدست آوردن اطلاعات هنرجو بعد از اخرین عملکرد حضور و غیاب در باشگاه
      SELECT @PrntCont = M.PRNT_CONT
        ...
        FROM Member_Ship M, dbo.Fighter f
       WHERE f.FILE_NO = m.FIGH_FILE_NO
         AND f.MBFZ_RWNO_DNRM = m.RWNO
         AND m.RECT_CODE = '004'
         AND f.FILE_NO = @fileno;    
