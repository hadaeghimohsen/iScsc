...
(
	...
	SELECT RQTP.CODE AS RQTP_CODE, RQTP.RQTP_DESC, RQTP.MUST_LOCK AS RQTP_MUST_LOCK, RQTT.RQTT_DESC, RQTT.CODE AS RQTT_CODE, RQRQ.SUB_SYS AS RQRQ_SUB_SYS, 
          ...
          EXPN.EXTR_PRCT AS EXPN_EXTR_PRCT, EXPN.EXPN_STAT, EXPN.ADD_QUTS, CTGY.NAME AS CTGY_NAME, CTGY.CTGY_DESC, CTGY.CODE AS CTGY_CODE, MTOD.CODE AS MTOD_CODE, MTOD.MTOD_DESC, 
    ...
    WHERE (RQRQ.PERM_STAT      = '002') 
      ...
      AND (@MtodCode IS NULL OR MTOD.CODE      = @MtodCode)
      AND (@CtgyCode IS NULL OR CTGY.CODE      = @CtgyCode)
)
GO
