...
--           T.CASH_CODE,
           ...
           --T.FIGH_FILE_NO,
           T.TOTL_AMNT,
           ISNULL((SELECT SUM(pm.AMNT) FROM Payment_Method Pm WHERE pm.PYMT_CASH_CODE = T.CASH_CODE AND pm.PYMT_RQST_RQID = T.PYMT_RQST_RQID) , 0) AS TOTL_RCPT_AMNT,
           ISNULL((SELECT SUM(pc.AMNT) FROM Payment_Discount pc WHERE pc.PYMT_CASH_CODE = T.CASH_CODE AND pc.PYMT_RQST_RQID = T.PYMT_RQST_RQID), 0) AS TOTL_DSCT_AMNT
     FROM (
   SELECT  r.RQTP_CODE, 
           rqtp.RQTP_DESC, 
           r.RQTT_CODE,
