...
[FGPB_CTGY_CODE_DNRM] [bigint] NULL,
[FGPB_MTOD_CODE_DNRM] [bigint] NULL,
[FGPB_CBMT_CODE_DNRM] [bigint] NULL,
[FGPB_COCH_FILE_NO] [bigint] NULL,
   ...
   WHEN MATCHED THEN
      UPDATE
         SET T.MDFY_BY   = UPPER(SUSER_NAME())
            ,T.MDFY_DATE = GETDATE()
            ,T.NUMB_OF_MONT_DNRM = CASE 
                                    WHEN DATEDIFF(DAY, S.STRT_DATE, S.END_DATE) < 30 THEN 1
                                    ELSE DATEDIFF(MONTH, S.STRT_DATE, S.END_DATE) 
                                 END
            ,T.NUMB_OF_DAYS_DNRM = DATEDIFF(DAY, S.STRT_DATE, S.END_DATE) + 1
            ,T.FGPB_CTGY_CODE_DNRM = CASE WHEN ISNULL(S.FGPB_RWNO_DNRM, 0) >= 1 THEN (SELECT fp.CTGY_CODE FROM dbo.Fighter_Public fp WHERE fp.FIGH_FILE_NO = s.FIGH_FILE_NO AND fp.RECT_CODE = '004' AND fp.RWNO = s.FGPB_RWNO_DNRM) END 
            ,T.FGPB_MTOD_CODE_DNRM = CASE WHEN ISNULL(S.FGPB_RWNO_DNRM, 0) >= 1 THEN (SELECT fp.MTOD_CODE FROM dbo.Fighter_Public fp WHERE fp.FIGH_FILE_NO = s.FIGH_FILE_NO AND fp.RECT_CODE = '004' AND fp.RWNO = s.FGPB_RWNO_DNRM) END 
            ,T.FGPB_CBMT_CODE_DNRM = CASE WHEN ISNULL(S.FGPB_RWNO_DNRM, 0) >= 1 THEN (SELECT fp.CBMT_CODE FROM dbo.Fighter_Public fp WHERE fp.FIGH_FILE_NO = s.FIGH_FILE_NO AND fp.RECT_CODE = '004' AND fp.RWNO = s.FGPB_RWNO_DNRM) END 
            ,T.FGPB_COCH_FILE_NO   = CASE WHEN ISNULL(S.FGPB_RWNO_DNRM, 0) >= 1 THEN (SELECT fp.COCH_FILE_NO FROM dbo.Fighter_Public fp WHERE fp.FIGH_FILE_NO = s.FIGH_FILE_NO AND fp.RECT_CODE = '004' AND fp.RWNO = s.FGPB_RWNO_DNRM) END 
...
GO
ALTER TABLE [dbo].[Member_Ship] ADD CONSTRAINT [FK_MBSP_COCH] FOREIGN KEY ([FGPB_COCH_FILE_NO]) REFERENCES [dbo].[Fighter] ([FILE_NO])
GO
ALTER TABLE [dbo].[Member_Ship] ADD CONSTRAINT [FK_MBSP_CTGY] FOREIGN KEY ([FGPB_CTGY_CODE_DNRM]) REFERENCES [dbo].[Category_Belt] ([CODE])
GO
...
GO
ALTER TABLE [dbo].[Member_Ship] ADD CONSTRAINT [FK_MBSP_MTOD] FOREIGN KEY ([FGPB_MTOD_CODE_DNRM]) REFERENCES [dbo].[Method] ([CODE])
GO
