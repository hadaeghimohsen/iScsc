...
SET ANSI_NULLS ON
GO
CREATE FUNCTION [dbo].[VFT$Last_Info_Deleted_Fighter] (
   @FileNo BIGINT, 
...
FROM         dbo.Fighter INNER JOIN
             ...
             dbo.D$ACTG ON ISNULL(dbo.Fighter_Public.ACTV_TAG, '001') = dbo.D$ACTG.VALU LEFT OUTER JOIN
...
WHERE     (dbo.Fighter_Public.RECT_CODE = '004') AND (dbo.Fighter.CONF_STAT = '002')
  ...
  AND     (dbo.Fighter.ACTV_TAG_DNRM <= '100')
