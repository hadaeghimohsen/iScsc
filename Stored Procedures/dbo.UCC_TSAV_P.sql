...
BEGIN
	...
	BEGIN TRY
      ...
      IF EXISTS(SELECT * FROM dbo.Club_Method cm, dbo.Settings s WHERE cm.CLUB_CODE = s.CLUB_CODE AND cm.CODE = @CbmtCode AND s.SHAR_MBSP_STAT = '002') AND @GlobCode IS NOT NULL AND @GlobCode != '' AND LEN(@GlobCode) > 2
      BEGIN         
