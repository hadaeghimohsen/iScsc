...
BEGIN
   ...
   BEGIN
      ...
      DECLARE @p0 SMALLINT = SUBSTRING(dbo.GET_MTOS_U(GETDATE()), 1, 4),@p1 varchar(3) = '001',@p3 DATETIME = GETDATE(),@p4 nvarchar(4000) = 'RelaySoft',@p5 DATETIME = GETDATE(),@p6 DATETIME = DATEADD(YEAR, 1, GETDATE()),@p7 REAL = 0,@p8 REAL = 0,@p9 varchar(3) = '002',@p10 BIGINT;      
		...
		EXEC [dbo].[INS_CTGY_P] @Mtod_Code = @MtodCode, @Name = NULL, @Ctgy_Desc = N'سبد خرید', @Ordr = NULL, @Epit_Type = '001', @Numb_Of_Attn_Mont = 0, @Numb_Cycl_Day = 0, @Numb_Mont_Ofer = 0, @Prvt_Coch_Expn = NULL, @Pric = 0, @Dflt_Stat = '001', @Ctgy_Stat = '002', @Gust_Numb = 0, @Natl_Code = NULL, @Rwrd_Attn_Pric = 0, @Show_Stat = '002';
