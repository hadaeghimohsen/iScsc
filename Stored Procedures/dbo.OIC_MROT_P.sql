...
BEGIN
   ...
   END
	...
	BEGIN TRY
      ...
      IF @TotlSesn = 0 BEGIN RAISERROR(N'ورود تعداد جلسات الزامی می باشد', 16, 1); RETURN; END
      IF @TotlSesn <= 1 BEGIN RAISERROR(N'تعداد جلسات باید بیشتر یک جلسه باشد', 16, 1); RETURN; END
      IF EXISTS(
         SELECT * 
           FROM Fighter
      ...
      IF @BrthDate = '1900-01-01' BEGIN RAISERROR(N'ورود تاریخ تولد الزامی می باشد', 16, 1); RETURN; END
      ...
      BEGIN
         ...
         VALUES(@FileNo, '001', @MbspRwno, @Snid, '002', @ExpnCode, @CardNumb, @TotlSesn, @CbmtCode);
      END
      ELSE
         UPDATE [Session]
