...
BEGIN
   ...
   BEGIN
      ...
      DECLARE @p0 SMALLINT = SUBSTRING(dbo.GET_MTOS_U(GETDATE()), 1, 4),@p1 varchar(3) = '001',@p3 DATETIME = GETDATE(),@p4 nvarchar(4000) = 'RelaySoft',@p5 DATETIME = GETDATE(),@p6 DATETIME = DATEADD(YEAR, 1, GETDATE()),@p7 REAL = 0,@p8 REAL = 0,@p9 varchar(3) = '002',@p10 BIGINT;      
		...
		EXEC dbo.INS_MTOD_P @Mtod_Desc = N'فروشگاه اینترنتی',@Mtod_Code = NULL,@Epit_Type = '001',@Dflt_Stat = '001',@Mtod_Stat = '002',@Chck_Attn_Alrm = '001',@Natl_Code = '',@Shar_Stat = '001', @Show_Stat = '002';
		
		DECLARE @MtodCode BIGINT;
		SELECT TOP 1 @MtodCode = CODE FROM dbo.Method;
		EXEC [dbo].[INS_CTGY_P] @Mtod_Code = @MtodCode, @Ctgy_Code = NULL, @Name = NULL, @Ctgy_Desc = N'سبد خرید', @Ordr = NULL, @Epit_Type = '001', @Numb_Of_Attn_Mont = 0, @Numb_Cycl_Day = 0, @Numb_Mont_Ofer = 0, @Prvt_Coch_Expn = NULL, @Pric = 0, @Dflt_Stat = '001', @Ctgy_Stat = '002', @Gust_Numb = 0, @Natl_Code = NULL, @Rwrd_Attn_Pric = 0, @Show_Stat = '002', @Fee_Amnt = NULL;
