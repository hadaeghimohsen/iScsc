...
-- =============================================
CREATE PROCEDURE [dbo].[MBS_SAVE_P]
	...
	*/
	...
	BEGIN TRY
      ...
      DECLARE @StrtDate DATE
             ...
             ,@SesnMeetDate DATETIME
             ,@SesnMeetType VARCHAR(3)
             ,@SmsSend VARCHAR(3);
      ...
      SELECT @StrtDate = M.STRT_DATE
            ...
            ,@SesnMeetDate = M.SESN_MEET_DATE
            ,@SesnMeetType = M.SESN_MEET_TYPE
            ,@SmsSend = M.SMS_SEND
        FROM Member_Ship M
      ...
      IF NOT EXISTS(SELECT * FROM Member_Ship WHERE RQRO_RQST_RQID = @Rqid AND RQRO_RWNO = @RqroRwno AND FIGH_FILE_NO = @FileNo AND RECT_CODE = '004')
         EXEC INS_MBSP_P @Rqid, @RqroRwno, @FileNo, '004', '006', @StrtDate, @EndDate, @PrntCont, @NumbMontOfer, @NumbOfAttnMont, @NumbOfAttnWeek, @AttnDayType;
      ELSE
         EXEC UPD_MBSP_P @Rqid, @RqroRwno, @FileNo, '004', '006', @StrtDate, @EndDate, @PrntCont, @NumbMontOfer, @NumbOfAttnMont, @NumbOfAttnWeek, @AttnDayType;
      
      UPDATE dbo.Member_Ship
         SET SESN_MEET_DATE = @SesnMeetDate
            ,SESN_MEET_TYPE = @SesnMeetType
            ,SMS_SEND = @SmsSend
